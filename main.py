...
def _write_runtime_evidence() -> None:
    ...
    evidence = {
        ...
        "probes": [
            {"name": probe.name, "available": probe.available, "output": probe.output, "error": probe.error}
...
if os.environ.get("ANDROID_ARGUMENT"):
    ...
    class MonitorApp(App):
        def build(self):
            self.status = Label(text="YJ-64: готов", halign="left", valign="top")
            ...
            start_button = Button(text="Запустить монитор", size_hint_y=None, height=60)
            start_button.bind(on_release=self.start_monitor)
            root.add_widget(start_button)

            stop_button = Button(text="Остановить монитор", size_hint_y=None, height=60)
        ...
        def start_monitor(self, *_):
            ...
            self.status.text = "Монитор запущен. Теперь можно запускать целевое приложение."
