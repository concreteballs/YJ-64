...
class DiagnosticQueryGenerator:
    """Generate diagnostic inquiries from configured templates."""

    def __init__(self, config: DiagnosticConfig, rng: random.Random | None = None) -> None:
        self._queries = config.queries
        self._rng = rng or random.Random()

    def generate(self, state_drift: float = 0.0) -> str:
        """Select a query, optionally preferring diagnostic prompts during drift."""
